...
const configured = Boolean(
  config.apiKey &&
  !String(config.apiKey).startsWith("PEGA_AQUI") &&
  config.databaseURL &&
  !String(config.databaseURL).includes("TU-PROYECTO")
);
...
async function initFirebase() {
  ...
  try {
    ...
    db = null;
    firebaseApi = null;
    setStatus("No se pudo iniciar Firebase. El juego funciona, pero el ranking quedará en modo local en este navegador.", "danger");
    loadLeaderboard();
  }
}
...
function startQuiz() {
  ...
  if (!bank.length) {
    alert("No se encontró el banco de preguntas. Revisa que el archivo questions.js esté en la misma carpeta que index.html.");
    return;
  }
...
function renderQuestion() {
  ...
  if (!q) {
    alert("No se pudo cargar la pregunta. Revisa el archivo questions.js.");
    return;
  }
...
async function finishQuiz() {
  ...
  try {
    ...
    console.error("No se pudo guardar en Firebase. Se guardará localmente:", error);
    saveResultLocal(result);
    setStatus("No se pudo guardar en Firebase. El resultado quedó guardado solo en este navegador.", "danger");
  }
...
async function saveResult(result) {
  if (db && firebaseApi) {
    const id = firebaseApi.push(firebaseApi.ref(db, "results")).key;
    await firebaseApi.set(firebaseApi.ref(db, `results/${id}`), {
      ...result,
...
function loadLeaderboard() {
  if (db && firebaseApi) {
    if (!leaderboardListenerStarted) {
      leaderboardListenerStarted = true;
      firebaseApi.onValue(
        firebaseApi.ref(db, "results"),
        snap => renderLeaderboard(snapshotToArray(snap.val())),
        { onlyOnce: false }
      );
    }
  } else {
...
function renderLeaderboard(rows) {
  ...
  const sorted = [...rows]
    .sort((a, b) => (b.score - a.score) || (b.percentage - a.percentage) || (a.finishedAt - b.finishedAt))
    .slice(0, 30);
  ...
  board.innerHTML = sorted.map((r, i) => `
    <div class="rank-row">
      <strong>#${i + 1}</strong>
      <div>
        <div class="rank-name">${escapeHtml(r.name || "Estudiante")}</div>
        <div class="rank-meta">${escapeHtml(r.course || "Sin curso")} · ${formatDate(r.finishedAt)}</div>
      </div>
